...
| C6 | 详情页侧边栏 vs 产品详情模板无侧边栏 | 第116行 vs 第128–138行 | 产品详情按模板10.1用**底部"配套料号"轮播**承担"相关"职能；方案/支持详情用**侧边栏相关条目**。**⚠️ 已于开发后期被用户明确反馈反转**——product-detail 现补充"产品分类导航"侧边栏，轮播保留，见 design.md §5.4"设计变更" |
| C7 | 新闻详情"单栏居中" vs "侧边栏新闻导航" | 第29/116行 vs 第151–158行（模板10.3） | 新闻详情**单栏居中**（取模板10.3，无左侧边栏）；"新闻导航/相关"由**底部"Latest News"3卡**（最近3篇，排除当前，任意类型）承担。**⚠️ 已于开发后期被用户明确反馈反转**——news-detail 现补充"新闻分类导航"侧边栏，底部3卡保留，见 design.md §5.12"设计变更" |
...
- 侧边栏规则（解决 C6/C7，⚠️ 开发后期两次被用户明确反馈修订，最终口径）：产品二级分类页、方案详情、支持详情、**产品详情、新闻详情**含侧边栏（分类导航/相关条目/TOC）；`products-list`/`solutions-list`/`support-list`/`news-list` 四个**列表页均无侧边栏**（与页面自身的分类卡网格/Tab 重复，见 design.md §4.4）。C6/C7 原裁定（产品详情/新闻详情无侧边栏）已被反转，见 design.md §5.4/§5.12"设计变更"。
...
- **功能**（news-detail 模板）：Header Banner（全宽背景图/色块叠加 H1 + 日期 + 分类标签，文字带阴影）；正文 + 侧边栏（News Categories + Explore More，⚠️ 开发后期反转 C7 原"无左侧边栏"裁定，用户明确反馈要求补充，见 design.md §5.12"设计变更"）；社交分享栏；底部"Latest News"3 卡（最近 3 篇，排除当前，任意类型）；NewsArticle JSON-LD；结尾 CTA。
- **场景**：访客读行业动态（含权威来源引用 + 本站解读），获得富媒体摘要点击。
- **验收**
  - [ ] 4 篇新闻详情发布（含公司新闻与行业动态）。
  - [ ] Banner 叠加标题/日期/标签且可读；正文 + 侧边栏布局。
  - [ ] 侧边栏（News Categories + Explore More）+ 分享栏 + 底部"Latest News"3 卡（最近 3 篇，排除当前，任意类型）齐全。
...
- **功能**：**独立页 + 独立 `contact` 模板**（共享 nav/footer/breadcrumb/Meta 占位符）；现代网格布局；Global Sales Network 全球办公网络（见下）；右侧固定联系浮层。⚠️ **开发后期两次修订，最终口径**：最初实现为询价表单（姓名/邮箱/型号/留言）+ WhatsApp/WeChat 联系卡片；`contact.txt` 后来在工作区外提供，解析后新增 Global Sales Network 区块；随后用户明确反馈去掉询价表单和 WhatsApp/WeChat 联系卡片，故当前页面仅剩 H1 + Global Sales Network + 地图占位/营业说明，联系方式改由全站浮层承担，见 design.md §5.14"设计变更"。
- **数据来源（条件式，依 prompt 第119行）**：若工作区存在 `contact.txt` → 解析其联系方式并同步进 `site.json.contact`。**该条件后来触发**——用户提供的 `contact.txt`（总部+3个中国办公室+15个国际办公室，共19个 office）已解析进 `site.json.contact.globalNetwork`（4 个 region）。WhatsApp +86 15013702378、WeChat +86 18612518271 仍保留在 `site.json.contact` 供 nav/footer/浮层复用。
- **场景**：用户查找最近的区域办公室，或通过浮层发起 WhatsApp/WeChat 联系。
- **验收**：Contact 为独立模板生成的独立页；Global Sales Network 数据取自 `site.json` 并正确显示（4 region、19 office）；联系浮层可用；无空链接。
...
- **模板清单（11 套 prompt 点名 + Contact = 12 套）**：
  ...
  - 所有模板共享 nav/footer/breadcrumb/Meta 占位符；**侧边栏组件**（⚠️ 开发后期修订，最终口径见 §3.1）用于产品分类页/方案详情/支持详情/**产品详情/新闻详情**；`products-list`/`solutions-list`/`support-list`/`news-list` 四个列表页均不用（与页面自身分类卡网格/Tab 重复）。
