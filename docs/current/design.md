...
### 4.4 侧边栏 Sidebar（分类页/产品详情页/方案详情页/新闻详情页/支持详情页）
- 产品分类导航树（高亮当前分类）；新闻详情页为 News Categories + Explore More。
- 详情页：相关条目列表 / 支持详情含 Sticky TOC（`position:sticky;top:100px`，z-10）。
- 移动端：移到主内容下方（折叠面板）。
- **不含侧边栏的页面**（均按用户明确反馈去除，因与页面自身的分类导航/卡片网格重复）：`products-list`（§5.2）、`solutions-list`（§5.5）、`support-list`（§5.7，已有 Tab）、`news-list`（§5.11）。
...
### 4.11 表单（询价/向工程师提问）— ⚠️ 已废弃
字段：Name / Email / Part No. / Message；每字段有 `<label for>`；必填校验 + 错误提示就近显示（红字+边框）。提交按钮为 Primary CTA，提交态禁用+spinner。纯前端校验（静态站，提交后展示成功提示/邮件链接）。**开发后期按用户明确反馈从 `/contact/` 移除**（连同 WhatsApp/WeChat 联系卡片），`src/assets/js/form.js` 已删除，见 §5.14 末尾说明与 `markup-contract.md` §4。
...
> 通用：所有模板含 Nav / Footer / 右侧联系浮层 / SEO Meta 占位符；**面包屑仅非首页模板**（首页 home 无面包屑）。侧边栏原按 §3.1 PRD 规则（news/product detail 无侧边栏）；**该规则已被用户在开发后期反转，news-detail / product-detail 现均含侧边栏，见 5.4/5.12 末尾"设计变更"说明**。同期用户另反馈 `products-list`/`solutions-list`/`support-list`/`news-list` 四个列表页的侧边栏与页面自身的分类导航/Tab/卡片网格重复，已全部移除，改为单栏，见 §4.4 末尾条目与各自 §5.2/5.5/5.7/5.11。
...
Breadcrumb → H1 + 200字行业介绍 → 4 分类大卡（图标+简介+"View Models →"）网格。单栏，无侧边栏（原方案含分类导航侧边栏，已按用户明确反馈移除——4张分类卡本身即分类导航，见 §4.4）。
...
### 5.4 产品详情页 `product-detail`（电商级；侧边栏见下方"设计变更"）
```
[Breadcrumb]
┌── 左: 产品大图区 ──┬── 右: 核心信息栏 ───────────────┐
│  主图(SVG)         │ H1 型号                          │
...
**设计变更**：原方案（PRD C6）为无侧边栏，靠 Tab + 底部配套料号轮播承担"相关"职能。用户在开发后期直接反馈要求补充侧边栏，明确指定内容为"产品分类导航"（与 `product-category.html` 侧边栏同一 `sidebarNav('Product Categories', ...)` 调用，当前分类高亮），已实现为 `.layout-with-sidebar` 布局，Tab/轮播保留不变。

### 5.5 解决方案列表页 `solutions-list`
Breadcrumb → H1 → 方案卡列表（每卡：行业图标 + 标题 + Summary 摘要 + "Read →"）。单栏，无侧边栏（原方案含行业导航侧边栏，已按用户明确反馈移除——方案卡网格本身即行业导航，见 §4.4）。
...
Breadcrumb → H1 → 两分区：**Company News** 与 **Industry News**（分区标题 + 各自卡片网格，不混排）。单栏，无侧边栏（原方案含新闻导航侧边栏，已按用户明确反馈移除——两个分区标题本身即新闻导航；分区 `id="company-news"`/`id="industry-news"` 保留供 news-detail 侧边栏锚点跳转使用，见 §5.12）。

### 5.12 新闻详情页 `news-detail`（杂志风；侧边栏见下方"设计变更"，原依 C7 无侧边栏）
```
...
**设计变更**：原方案（PRD C7）为单栏居中、无侧边栏，靠底部"Latest News"3卡承担新闻导航职能。用户在开发后期直接反馈要求补充侧边栏，明确指定内容为"新闻分类导航"（Company News / Industry News，锚点跳转至 `/news/` 对应区块），与 `/news/` 列表页侧边栏同一结构（另加"Explore More"跨栏目导航），已实现为 `.layout-with-sidebar` 布局，Banner 全宽/底部3卡保留不变。

### 5.13 关于我们 `about`
Breadcrumb → Hero 简介 → 公司简介&发展历程（时间线）→ 优势 Feature Grid → 客户案例 Logo 墙 → 报关单展示区（信任）→ 团队/FAE 入口 → CTA。模块化网格布局。

### 5.14 联系我们 `contact`（独立模板）
Breadcrumb → H1 → Global Sales Network（各地区办公室网格卡片，见下方"设计变更"）→ 底部地图占位/营业说明。右侧浮层同全站。

**设计变更**：原方案为"左 联系卡（WhatsApp/WeChat + 二维码 SVG）/ 右 询价表单"网格。开发后期按用户明确反馈，contact.txt（工作区外部提供的全球办公网络文件）解析后新增 Global Sales Network 区块（4 地区/19 办公室，见 §37 CSS/`contact.html`），随后用户又明确要求去掉左侧联系卡（"Talk to BeiLuo Directly"）和右侧询价表单（"Request a Quote"），故当前页面仅保留 Global Sales Network + 底部地图占位/营业说明 + 全站浮层，WhatsApp/WeChat 联系方式改由浮层承担。`form.js` 已删除，见 `markup-contract.md` §4。
