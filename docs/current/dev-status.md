...
> Last updated: 2026-07-05 (T8.2's 3 most-severe fixes committed (e95bc7f); §6.A's orphan-field wiring batch (nav/footer/contact/og:site_name/Organization JSON-LD) implemented/reviewed/Codex-approved, pending commit; T8.2 itself remains **in_progress**, ~28 smaller findings still recorded as candidates — see §6; T8.1 still paused per user decision, resume later)
...
- Working tree: **dirty** — T8.2's §6.A orphan-field-wiring batch (`src/data/site.json`, `src/lib/pages.js`, `src/templates/home.html`, `src/templates/partials/{nav,footer,contact-float,head-meta}.html`) is complete and Codex-approved but **not yet committed**, pending user confirmation. T8.2's 3 severe fixes from the previous batch are already committed (e95bc7f).
...
- ~~`nav.items`/`navCategories`/`footer.columns`/`contact.whatsappHref`/`seo.siteName` all orphaned (never rendered); `Organization` JSON-LD missing `logo`/`contactPoint`, wrong `description` source field~~ — fixed T8.2 (pending commit): nav/footer/contact-float/head-meta/home.html all made data-driven, see §3/§6.A.
- ~~`pages.js`: home/about pages' `seo` context object silently lost `siteName`/`defaultTitle`/`defaultDescription`/`baseUrl` due to a shallow-spread key collision with their own `home.json`/`about.json` flat `seo` field~~ — fixed T8.2 (pending commit): discovered as a side effect of the `og:site_name` fix above, not a pre-planned finding.
...
**T8.2's 3 severe fixes are committed; the §6.A orphan-field-wiring batch is done, pending commit.** Both T8.1 and T8.2 are now `pending`/`in_progress` with real remaining work explicitly deferred by user decision — this is intentional, not stalled. Ask the user which to resume (T8.1's CSS gap, T8.2's ~28 remaining smaller findings, or a different task like T9.x) rather than silently picking one.
...
1. §6.A's orphan-field wiring is now done except the WeChat-QR-toggle (deliberately deferred, needs new JS/CSS/design decisions — recommend its own small task).
2. Next biggest win-per-effort: §6.B's content-quality tweaks (hero headline, 2 teaser summaries, news voice), then §6.C's count-quota fixes (solutions bomList/industry coverage), then §6.D's smaller placeholder issues.
3. Re-run the check_list2.md audit for real completion, and only then mark T8.2 `completed`.

Also before/during T9.1: resolve the remaining dead links (currently 4, down from ~17–25 after T8.1/T8.2's fixes) and ~13+ missing SVG/PDF assets (see §6.E of both T8.1 and T8.2 findings) — T9.1's own acceptance criterion is zero dead links, so these need resolving by then regardless.
