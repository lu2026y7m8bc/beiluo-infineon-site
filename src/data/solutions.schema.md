...
| `icon` | String | Required | Per-industry 24×24 line-icon SVG path (design §9 unified icon style), distinct per solution — not the shared `diagramSrc` illustration | `{{icon}}` as `<img>` on `solutions-list` card (design §5.5, check_list1 D5) |
| `diagramSrc` | String | Required | System block diagram SVG path (ideally per-slug, e.g. `"/assets/svg/illustrations/bd-<slug>.svg"` — all 6 solutions currently share the same generic `solution-diagram.svg` pending distinct diagram assets, tracked in dev-status.md §6) | `{{solution.diagramSrc}}` as `<img src>` in first H2 section (design §5.6) |
